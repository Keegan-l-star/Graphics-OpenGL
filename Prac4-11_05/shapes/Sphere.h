...
public:
    ...
    // Normal helpers
    Vector<n> normalAtParams(float phi, float theta) const; // phi: latitude, theta: longitude
    Vector<n> normalAtPoint(const Vector<n>& p) const; // normal for given point on sphere surface
